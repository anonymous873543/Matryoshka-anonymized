...
run-exp () {
  ...
  local totalSize=$2
  local numGroups=$3
  ./exp-spark5.sh $resultDir/result-$classname-$totalSize-$numGroups.csv conf/nested/conf_BounceRateSpark anonymous.bouncerate.$classname TOBEFILLED.jar $totalSize $numGroups
}
