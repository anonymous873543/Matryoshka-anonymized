...
run-exp () {
  ...
  ./exp-spark5.sh $resultDir/result-$classname-$LBLSJoinStrategy-$days-$numVertices-$numEdges.csv conf/nested/conf_PageRankSpark anonymous.pagerank.$classname TOBEFILLED.jar $epsilon $days $numVertices $numEdges $LBLSJoinStrategy
}
