...
run-exp () {
  ...
  local numEdges=$4
  local sampleSize=$5
  ./exp-spark5.sh $resultDir/result-$classname-$numComp-$numVert-$numEdges-$sampleSize.csv conf/nested/conf_AvgDistancesSpark anonymous.avgdistances.$classname TOBEFILLED.jar $numComp $numVert $numEdges $sampleSize
}
