...
run-exp () {
  ...
  ./exp-spark5.sh $resultDir/result-$classname-$withClosureStrategy-ac$autoCoalesce-$numPoints-$K-2-$numRuns.csv conf/nested/conf_KMeansSpark anonymous.kmeans.$classname TOBEFILLED.jar $numPoints $K 2 $numRuns $withClosureStrategy
}
